...
class MLPRegression(nn.Module):
    def __init__(self, input_dims=10, output_dims=1, mlp_layers=[128, 128, 128, 128, 128],skips=[2], act_fn=ReLU, nerf=True, normalize=True):
    ...
    def forward(self, x):
        ...
        if self.normalize:
            x = (x - torch.tensor([-torch.pi, -torch.pi, -torch.pi, -torch.pi], dtype=torch.float32, device='cuda')) \
                / torch.tensor([2 * torch.pi, 2 * torch.pi, 2 * torch.pi, 2 * torch.pi], dtype=torch.float32, device='cuda') ## for joint space
...
class MLPRegression_IK(nn.Module):
    def __init__(self, input_dims=10, output_dims=1, mlp_layers=[128, 128, 128, 128, 128],skips=[2], act_fn=ReLU, nerf=True, normalize=True):
        """Create an instance of mlp nn model
        Args:
            input_dims (int): number of channels
            ...
            nerf (bool, optional): use positional encoding (x->[sin(x),cos(x)]). Defaults to False.
        """
        super(MLPRegression_IK, self).__init__()
    ...
    def forward(self, x):
        """forward pass on network."""
        if self.normalize:
            x = (x - torch.tensor([-torch.pi, -torch.pi, -3, -3], dtype=torch.float32, device='cuda')) \
                / torch.tensor([2 * torch.pi, 2 * torch.pi, 2 * 3, 2 * 3], dtype=torch.float32, device='cuda')
